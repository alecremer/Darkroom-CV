...
from yolo_dataset import YOLODetectionDataset, YOLOSegmentationDataset
...
def prepare_image_for_save(tensor):
    ...
    return unnormalize(tensor).clamp(0.0, 1.0)

def save_single_mae_visualization(orig_img, masked_img, recon_img, epoch, idx, output_dir="reconstructions"):
...
def visualize_mae_step(model, val_loader, epoch, device, output_dir="reconstructions"):
    """
    ...
    with torch.no_grad():
        for images_batch, _ in val_loader:
            ...
            # Salvar o primeiro exemplo do batch (índice 0)
                ...
                orig_img = prepare_image_for_save(x_original[idx])
                masked_img = prepare_image_for_save(im_masked[idx])
                recon_img = prepare_image_for_save(im_paste[idx])

                save_single_mae_visualization(orig_img, masked_img, recon_img, epoch, all_images_processed, output_dir)
...
def train_mae(dataset: str):

    model_dir = "mae_checkpoints"
    ...
    val_dataset = YOLOSegmentationDataset("/home/ale/Downloads/PI2/seg_50_stringing", image_size, transform, split="test") # Adiciona split
