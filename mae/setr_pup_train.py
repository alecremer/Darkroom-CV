...
def visualize_segmentation_val(model, val_loader, save_path, epoch, device):
    """
    ...
    with torch.no_grad():
        for i, (images, masks) in enumerate(val_loader):
            ...
            for idx in range(images.size(0)):
                ...
                img_tensor = unnormalize_tensor(images[idx]).cpu().clamp(0.0, 1.0)
...
def save_model_checkpoint(model, path, epoch, loss):
...
def fine_tune(dataset_path: str):
    ...
    # 1. Carregar o modelo MAE Encoder + Head de Segmentação
    model = load_mae_segmenter().to(device)

    # 2. Preparar os dados (ADAPTE AQUI COM SEUS CAMINHOS REAIS)
    ...
    # CrossEntropyLoss é padrão para segmentação
    criterion = nn.CrossEntropyLoss().to(device)
    
    # # Learning Rate menor para fine-tuning (geralmente 1e-4 ou 1e-5)
    FT_LR = 1e-4 
    FT_LR_HEAD = 1e-4    # LR mais alto para o Decoder (Head)
    FT_LR_ENCODER = 1e-5 # LR mais baixo para o Encoder MAE

    # Cria os grupos de parâmetros
    param_groups = [
        # Encoder: ajuste fino
        {'params': model.encoder.parameters(), 'lr': FT_LR_ENCODER},
        # Head (Decoder): aprendizado mais rápido
    ...
    print(f"Iniciando Fine-Tuning com LR: {FT_LR}")
    ...
    for epoch in range(FINE_TUNE_EPOCHS):
        ...
        #      model.train()
        if (epoch + 1) % visualize_epochs == 0 or epoch == FINE_TUNE_EPOCHS - 1:
            visualize_segmentation_val(model, val_loader, VISUALS_DIR, epoch + 1, device) # <-- Nova função

    print("\nFine-Tuning concluído!")
    print(f"Melhor perda (Loss): {BEST_LOSS:.4f}")
    
if __name__ == '__main__':
    fine_tune("/home/ale/Downloads/PI2/annotations_2025_10_02_03_49_06_yolo_1.1")